...
async function transcribeFileTrans(audioPath) {
  ...
  const transcript = await new Promise((resolve, reject) => {
    ...
    (u.protocol === 'https:' ? https : http).get(u, (resp) => {
      let d = '';
      resp.on('data', c => d += c.toString());
...
/**
 * Assign sentence text to shots based on word-level timestamps.
 * The sentence starts at its first word's begin_time, which determines
 * which shot it belongs to. If a sentence spans multiple shots, it goes to
 * the shot where it starts. Very long sentences spanning 3+ shots are
 * duplicated into each shot that has words from that sentence.
 */
function assignToShots(sentences, scenes) {
  ...
  if (scenes.length === 1) {
    results[0] = sentences.map(s => s.text).join('');
    return results;
  }

  // Build a map: for each shot, collect all sentences that start within it
  // If a sentence spans multiple shots, duplicate it for each affected shot
  sentences.forEach(sent => {
    if (!sent.words || sent.words.length === 0) return;
    const firstWordMs = sent.words[0].begin_time;
    const lastWordMs = sent.words[sent.words.length - 1].end_time;
    const startSec = firstWordMs / 1000;
    const endSec = lastWordMs / 1000;

    // Find all shots that overlap with this sentence's time range
    let assigned = false;
    for (let i = 0; i < scenes.length; i++) {
      const sceneStart = scenes[i].startTime;
      const sceneEnd = scenes[i].endTime;
      const overlap = Math.min(sceneEnd, endSec) - Math.max(sceneStart, startSec);
      if (overlap > 0) {
        if (results[i]) results[i] += sent.text;
        else results[i] = sent.text;
        assigned = true;
      }
      if (sceneStart > endSec) break;
    }

    // Fallback: sentence starts after last shot — assign to last shot
    if (!assigned && scenes.length > 0) {
      const last = scenes.length - 1;
      if (results[last]) results[last] += sent.text;
      else results[last] = sent.text;
    }
  });
